...
import { FaFacebook,FaGoogle,FaInstagram,FaYoutube, FaTrash, FaPen, FaSearch } from 'react-icons/fa';
...
class List extends React.Component {
    constructor(props) {
        super(props);
        this.state = { campaignList: [], FilterCampaignList: [], filter: false,searchCampaign:'' };
    }
    ...
    searchCampaigns = (searchValue) => {
        ...
        if (searchValue !== '') {
            ...
            let data = this.state.campaignList.filter(campaign => {
                ...
                return arr.some(el => campaign.Campaign_name.toLowerCase().includes(el));
            });
  ...
  return (
    <div className='card m-4'>
        ...
        <Modal.Footer>
          ...
          <Button onClick={(e)=>this.editThisItem(this.state.editItemId,this.state.ChampaignOnOff)} variant="danger">Confirm</Button>
        </Modal.Footer>
      </Modal>
        <div id="Filters" className='m-4 mb-1' >
            <Row>
                ...
                <Col xs={6} className="ms-auto">
                    <Row>
                    <Col xs={4}>
                    <Form.Select  onChange={(e) => this.filterData('Platform',e.target.value)} >
                    ...
                    <Col xs={4}>
                    <Form.Select onChange={(e) => this.filterData('Campaign_on',e.target.value)}>
                    ...
                    <Col xs={4}>
                    <Form.Select onChange={(e) => this.filterData('Days',e.target.value)}>
        ...
        <tbody>
            {(this.state.filter)?this.state.FilterCampaignList.map((campaign,id)=>(
                <tr>
                    <td>{id+1}</td>
                    <td>
                        <Form>
                    ...
                    <td>
                        <Row>
                            <Col xs="3">
                                <Image src={campaign.Campaign_image} thumbnail width={70}></Image>
                            </Col>
                            <Col xs>
                                <Row><h5>{campaign.Campaign_name}</h5></Row>
                                <Row><p>Created on{campaign.Created_on}</p></Row>
                            </Col>
                        </Row>
                    </td>
                    ...
                    <td>{campaign.Loc}</td>
                    <td>
                        {campaign.Platform == 'FB' ? <FaFacebook color='blue' size={30} /> :campaign.Platform == 'Google' ? <FaGoogle size={30} /> :campaign.Platform == 'Instagram' ? <FaInstagram color='red' size={30} />:campaign.Platform == 'Youtube' ? <FaYoutube color='red' size={30} />: null}
                    </td>
                    <td>
                        {campaign.Campaign_on ?<Badge bg="success">Live now</Badge>: <Badge bg="warning">Paused</Badge>}
                        
                    </td>
                    <td>
                        <FaPen color='blue' className='mx-2' onClick={(e) => this.EditCampaign(campaign._id,campaign.Campaign_on)} />
                        <FaTrash color='red' className='mx-2' onClick={()=>this.deleteCampaign(campaign._id)} />
                    </td>
                </tr>
            )):(this.state.campaignList != undefined && this.state.campaignList.length > 0 )? this.state.campaignList.map((campaign,id)=>(
                <tr>
                    <td>{id+1}</td>
                    <td>
                        <Form>
